...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    ...
    if (input.length == 0) ()
    else if (input.length == 1) output(0) = 0.0f
    else {
      val size = input.length

      @tailrec
      def recur(idx: Int, cMax: Float): Unit = {
        if (idx == size) ()
        else {
          val tng = input(idx) / idx
          val max = Math.max(cMax, tng)
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    ...
    @tailrec
    def recur(idx: Int, cMax: Float): Float = {
      if (idx >= until) cMax
      else {
        val tng = input(idx) / idx
        val max = Math.max(cMax, tng)
    ...
    recur(idx = from, cMax = input(from) / from)
  ...
  def upsweep(input: Array[Float], from: Int, end: Int,
    ...
    if (end - from <= threshold) Leaf(from, end, upsweepSequential(input, from, end))
      ...
      val (lMax, rMax) = parallel(upsweep(input, from, m, threshold), upsweep(input, m, end, threshold))
      Node(lMax, rMax)
    }
  }
  ...
  def downsweep(input: Array[Float], output: Array[Float], startingAngle: Float,
    ...
    case Leaf(from, until, _) =>
      downsweepSequential(input = input, output = output, startingAngle = startingAngle, from = from, until = until)
