...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics) {
  ...
  def updateBoundaries(boundaries: Boundaries, body: Body): Boundaries = new Boundaries {
    minX = Math.min(body.x, boundaries.minX)
    maxX = Math.max(body.x, boundaries.maxX)
    minY = Math.min(body.y, boundaries.minY)
    maxY = Math.max(body.y, boundaries.maxY)
  }
  ...
  def computeBoundaries(bodies: Seq[Body]): Boundaries = timeStats.timed("boundaries") {
    /*bodies foreach( body => println(s"(${body.x}, ${body.y}") )
    println()*/
