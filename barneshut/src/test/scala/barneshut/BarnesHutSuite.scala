...
import FloatOps._
  ...
  test("'SectorMatrix.+=' should add a body at (25,47) to the correct bucket of a sector matrix of size 96") {
    ...
    assert(sm(2, 3).size === 1 && sm(2, 3).find(_ == body).isDefined === true, s"Body not found in the right sector")
  }
  ...
  test(" 'computeSectorMatrix' should correctly work given 5 points within a boundary of size 96 when some points map to the same sector") {
    ...
    val seq = Seq(
      new Body(5, 12, 34, 0.1f, 0.1f),
      new Body(5, 23, 45, 0.1f, 0.1f),
      new Body(5, 56, 9, 0.1f, 0.1f),
      new Body(5, 8, 79, 0.1f, 0.1f),
      new Body(5, 8, 79, 0.1f, 0.1f),
      new Body(5, 8, 79, 0.1f, 0.1f),
      new Body(5, 5, 99, 0.1f, 0.1f)
    )
    ...
    val simulator = new Simulator(new ExecutionContextTaskSupport, new TimeStatistics)

    val matrix = simulator.computeSectorMatrix(seq, boundaries)

    println(s"sector size = ${matrix.sectorSize}")

    (matrix.matrix.indices zip matrix.matrix) foreach { case (index, elem) =>
      if (index % matrix.sectorPrecision == 0) println("")
      print(s"${elem.size} ")
    }
  ...
  test("'updateBoundaries' should correctly update the boundary when invoked repeatedly with points in the range (0,0) to (100,100)") {
    ...
    val bounds = seq.foldLeft(boundaries)((bnds, body) => simulator.updateBoundaries(bnds, body))

    println(s"bounds are: \n $bounds")
